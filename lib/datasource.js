...
DataSource.prototype.setup = function(name, settings) {
  ...
  if (name && !connector) {
    if (typeof name === 'object') {
      ...
      if (!connector) {
        debug('ERROR: The connector has not been resolved.', result.error);
  ...
  if (connector) {
    var postInit = function postInit(err, result) {
      ...
      if (this.connected) {
        ...
        debug('ERROR: Connection fails, it will be retried for the next request.', err);
        this.emit('error', err);
        this.connecting = false;
      }
    }.bind(this);
...
DataSource.prototype.idName = function(modelName) {
  if (!this.getModelDefinition(modelName).idName) {
    debug('ERROR: No id name', this.getModelDefinition(modelName));
  }
